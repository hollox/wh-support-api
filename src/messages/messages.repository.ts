...
import { convertRowsToModels, convertRowToModel } from "./messages.helper";
import { Message, MessageRecord } from "./messages.models";
...
export async function getByTicketId(ticketId: string): Promise<Message[]> {
  const query = {
    text:
      "SELECT message_id, ticket_id, author_user_id, content, creation_date FROM messages WHERE ticket_id = $1",
    values: [ticketId]
  };
