...
export async function getByOrganizationId(organizationId: string): Promise<Ticket[]> {
  const query = {
    text: `
SELECT
    ticket_id,
...
RETURNING
  ...
  title,
  content`,
    values: [ticket.ticketId, ticket.statusId, ticket.authorUserId, ticket.title, ticket.content, SYSTEM_UUID]
  };
}
