...
import * as repository from "../repository/organizations/organizations.repository";
...
export async function save(organization: Organization): Promise<Organization> {
  ...
  if (organization.organizationId) {
    savedOrganization.users = await usersService.getByOrganizationId(organization.organizationId);
  } else {
